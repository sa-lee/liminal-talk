...
- zooming to find hidden structuring
- higlighting within the tour path
...
- leverages client-side visualisation and interaction via `vegawidget`
- treats the basis interpolation/generation steps as a reactive stream  
]
...
.large[*Key idea*: instead of realising the entire tour path, 
generate new bases according to a desired frame rate. 
Allows user interactions to pause/restart the stream.
]
---
class: middle, center
...
.large[
- increasing interactivity increases code complexity
- large data = latency
- layers of complexity R <-> JS <-> vegalite
- ideally would like to do everything client side
]
...
# Global and local; embeddings with tours 
